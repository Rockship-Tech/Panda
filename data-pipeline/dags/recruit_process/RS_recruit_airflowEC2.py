...
def download_from_s3(s3_file):
    ...
    file_name = s3_hook.download_file(key=s3_file, bucket_name=bucket_name, preserve_file_name = True, use_autogenerated_subdir = True)
...
def upload_text_file(text, s3_file):
    ...
    s3_hook = S3Hook(aws_conn_id=AWS_CONN_ID)
    s3_hook.load_file(filename = local_text_path, key = key_s3, bucket_name = bucket_name, replace=True)
    logging.info(f"Uploaded text file to S3: {local_text_path} , " + f"key: {key_s3} , ")
